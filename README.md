...
```
 ...
 - Run virtme-ng with confidential guest support (currently only s390x support). **Use this only for test purposes!**
   ...
   Since this host-key document does not change very often, it makes sense to
   make use of [`virtme-ng.conf`](#default-options). To do so, modify the
   `default_opts` sections in `~/.config/virtme-ng/virtme-ng.conf` as follows:
   ...
   and start a confidential guest using the newly created boot image.
